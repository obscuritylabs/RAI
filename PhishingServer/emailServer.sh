...
echo "CREATE USER 'onlyoffice_user'@'localhost' IDENTIFIED BY 'TempP@ssW0rd12!';" | sudo tee $sqlcnf
echo "CREATE USER 'mail_admin'@'localhost' IDENTIFIED BY 'Isadmin123';" | sudo tee -a $sqlcnf
echo "GRANT ALL PRIVILEGES ON * . * TO 'root'@'%' IDENTIFIED BY 'TempP@ssW0rd12!';" | sudo tee -a $sqlcnf
echo "GRANT ALL PRIVILEGES ON * . * TO 'onlyoffice_user'@'%' IDENTIFIED BY 'TempP@ssW0rd12!';" | sudo tee -a $sqlcnf
echo "GRANT ALL PRIVILEGES ON * . * TO 'mail_admin'@'%' IDENTIFIED BY 'Isadmin123';" | sudo tee -a $sqlcnf
echo "FLUSH PRIVILEGES;" >> $sqlcnf

##### OnlyOffice config file
oocnf="/app/onlyoffice/mysql/conf.d/onlyoffice.cnf"

echo "[mysqld]" | sudo tee $oocnf
echo "sql_mode = 'NO_ENGINE_SUBSTITUTION'" | sudo tee -a $oocnf
echo "max_connections = 1000" | sudo tee -a $oocnf
echo "max_allowed_packet = 1048576000" | sudo tee -a $oocnf

sudo chmod -R 755 /app/

# Generate Self Signed Certificate
cd /app/onlyoffice/CommunityServer/data/certs || exit 1
